...
int main(int argc, char** argv) {
    ...
    for (auto& [key, c] : map) {
        ...
        // Непрозрачное, затем полупрозрачное. Воду рисовать
        // обязательно: без неё озеро выглядит дырой в земле, и
        // сравнивать снимок с устройством становится не с чем —
        // именно на этом разбор дыр и застрял однажды. Смешивание
        // упрощённое, вода поверх дна: оттенок важен, точная альфа
        // здесь ни к чему.
        for (u32 i = 0; i + 2 < opaque; i += 3)
            triangle(vv[idx[i]], vv[idx[i+1]], vv[idx[i+2]]);
        for (u32 i = opaque; i + 2 < (u32)idx.size(); i += 3)
            triangle(vv[idx[i]], vv[idx[i+1]], vv[idx[i+2]]);
    }
