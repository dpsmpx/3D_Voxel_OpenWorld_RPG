...
public:
    ...
    /// Отображение буфера в память процесса. Для hostVisible буфер
    /// отображается при создании и остаётся отображённым: повторный
    /// вызов просто возвращает тот же указатель. Для device-local
    /// возвращает nullptr — такую память отобразить нельзя.
    void* map();

    /// Снимает отображение. Нужен редко: постоянное отображение и есть
    /// задуманный режим работы. Вызвав его, помните, что писать в
    /// буфер после этого нельзя, пока снова не позвали map().
    void  unmap();
