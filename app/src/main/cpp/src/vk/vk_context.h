...
public:
    ...
    /// поведение. Задавали его не все проходы: вода и контур
    ...
    /// Проходы кадра — в том порядке, в каком они записываются.
    ///
    /// Порядок ОБЯЗАН совпадать с RenderSystem::render: длительность
    /// прохода считается как разность между его меткой и меткой
    /// предыдущего, а первый отсчитывается от начала кадра.
    enum class GpuPass : u8 {
        Terrain = 0,   ///< непрозрачный ландшафт
        Entities,      ///< NPC, мобы, снаряды, предметы
        Sky,           ///< небо, полноэкранный треугольник
        Water,         ///< полупрозрачный ландшафт
        Overlay,       ///< контур выделенного блока
        Ui,            ///< интерфейс
        Count
    };
    static constexpr u32 GPU_PASSES = (u32)GpuPass::Count;
    static const char*   passName(GpuPass p);

    /// Ставит метку «всё, что записано до этой точки, GPU закончил».
    ///
    /// Зовётся ПОСЛЕ команд прохода. Метка нижняя (BOTTOM_OF_PIPE):
    /// верхняя сработала бы, едва команда дошла до начала конвейера,
    /// то есть не дождавшись собственно рисования.
    ///
    /// ОГОВОРКА, без которой числа врут. Мобильный GPU — плиточный: он
    /// сначала раскладывает всю геометрию прохода рендера по плиткам и
    /// только потом красит плитку за плиткой. Фрагментная работа всех
    /// проходов при этом перемешана, и метка внутри прохода рендера
    /// показывает не «сколько красили небо», а «когда GPU добрался до
    /// этой точки в потоке команд». Числам можно верить как порядку
    /// величин и как относительному сдвигу между сборками, но не как
    /// раздельной стоимости.
    ///
    /// Честный способ — выключить проход целиком (render_passes в
...
private:
    ...
    f32                      lastGpuMs_ = 0.f;
    /// Меток на кадр: начало, конец и по одной на проход.
    static constexpr u32     STAMPS_PER_FRAME = 2 + GPU_PASSES;
    /// Сколько меток проходов уже записано в текущий командный буфер.
    /// Метка прохода, которую не записали, оставила бы в пуле
    /// неинициализированный запрос — vkGetQueryPoolResults вернул бы
