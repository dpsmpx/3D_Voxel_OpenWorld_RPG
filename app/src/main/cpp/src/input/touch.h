...
public:
    ...
    /// Убирает все экранные кнопки вместе с касаниями и перетаскиванием.
    ///
    /// Кнопки заводит игра при каждом появлении окна, а сам ввод живёт
    /// дольше окна. Без очистки каждое сворачивание добавляло второй
    /// набор кнопок поверх первого: касание доставалось старой кнопке
    /// (её находят первой), а удержание спрашивали у новой — и после
    /// первого же возврата в игру переставали работать удержание
    /// прыжка, атаки и защиты.
    void clearButtons();
    usize buttonCount() const { return buttons_.size(); }
