...
/// Название фракции для экрана репутации.
const char* factionName(FactionId id);
/// Название тира репутации: от «Ненавидят» до «Превозносят».
...
/// Цена убитого жителя.
///
/// Репутация умела только расти: единственным, кто её трогал, была
/// награда за квест, и то в плюс. Тиры Unfriendly, Hostile и Hated
/// были недостижимы вовсе — половина шкалы существовала на бумаге.
///
/// Число подобрано под сами пороги: одно убийство роняет в
/// Unfriendly, четыре — в Hostile, семь — в Hated. Резня в деревне
/// должна доводить до дна за один заход, а случайный удар по соседу
/// — нет, поэтому не сотня и не тысяча.
constexpr i32 REP_MURDER_PENALTY = 150;
...
/// Как тир репутации влияет на игру.
/// @param t достигнутый тир
/// @return множители цен, шанс нападения стражи, доступность квестов
