...
/// Локализованное сообщение о результате крафта — для строки статуса.
...
struct CraftContext {
    ...
    /// Прибавка к уровню ДЛЯ КРАФТА — узел «Craft Master».
    ///
    /// «Уровней крафта» отдельной шкалой в игре нет, и заводить её
    /// ради одного узла значило бы поставить рядом со шкалой уровня
    /// вторую, которая растёт от неё же. Прибавка идёт к тому
    /// уровню, по которому рецепт и проверяется: мастер берётся за
    /// рецепт на ранг раньше остальных.
