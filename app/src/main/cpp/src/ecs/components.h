...
// ============================================================
// ќсновные компоненты игрока, мобов, предметов.
// ¬се Ч POD по духу (простые структуры, тривиальное копирование).
// ============================================================
...
struct Experience {
    u64 current = 0;
    u32 level   = 1;
    // “аблица опыта дл€ уровн€: level^2 * 100
...
// Ч  оллайдер (дл€ физики)
struct Collider {
    glm::vec3 halfExtents{0.4f, 0.9f, 0.4f};
    bool      isStatic = false;
};

// Ч —сылка на модель / блок дл€ рендера
struct Renderable {
    u32 meshId = 0;
    u32 textureId = 0;
    glm::vec4 tint{1};
...
// Ч »дентификатор дл€ сохранени€/сериализации
struct PersistentId {
    u64 value = 0;
};
