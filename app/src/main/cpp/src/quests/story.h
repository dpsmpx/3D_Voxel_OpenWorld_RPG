...
/// Первая цель — та, ради которой игрок вообще куда-то идёт.
///
/// Без неё первые минуты выглядят так: лес, пустые руки, никого
/// вокруг и строка «текущее задание», которой нечего показать.
/// Сюжет при этом написан целиком — но его первая глава выдаётся
/// ТОЛЬКО через разговор с раздатчиком заданий, а узнать, что надо
/// искать людей, игроку было неоткуда.
///
/// Глава 0 на эту роль не годится: она ищет деревню «кроме той, в
/// которой стоим», то есть рассчитана на игрока, уже пришедшего к
/// людям. Здесь цель другая — найти людей ВООБЩЕ.
///
/// Если деревни рядом нет, целью становится дерево: топор начинается
/// с него, и это единственное, что можно сделать голыми руками.
/// Задание выдаётся сразу принятым — спрашивать разрешения не у кого.
///
/// @return сущность задания или недействительная, если журнал занят
