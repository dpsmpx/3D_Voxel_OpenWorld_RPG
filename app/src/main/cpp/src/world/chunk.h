...
struct Chunk {
    ...
    /// Какой уровень детализации нужен рендеру прямо сейчас.
    /// Мешер строит только его: раньше он строил все четыре и хранил
    /// квады всех четырёх, а нужен чанку почти всегда один.
    ...
    /// Уже стоит в очереди готовых мешей. Раньше дубликаты отсеивались
    /// линейным поиском по всей очереди под общим замком — из каждого
    /// рабочего потока, на каждый достроенный меш.
