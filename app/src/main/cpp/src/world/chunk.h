...
struct Chunk {
    ...
    /// Течение верхней грани воды, запечённое генерацией: по колонке
    /// (x * CHUNK_SIZE + z) — упакованные румб, скорость и бурление.
    ///
    /// Реки, озёра и море не текут сами: вода в мире не
    /// моделируется, и русло меняет только игрок. Поэтому течение
    /// известно заранее — оно пришло из графа стока вместе с руслом, —
    /// и пересчитывать его по соседним вокселям при каждой пересборке
    /// меша незачем. Оценка по рельефу (estimateWaterFlow) остаётся
    /// только для колонок, где игрок нарушил воду: там верхняя грань
    /// уже не на запечённой высоте.
    std::array<u8, CHUNK_SIZE * CHUNK_SIZE> waterFlow{};
