...
// —войства блока Ч компилируютс€ один раз, читаютс€ из рендера/физики
struct BlockDef {
    const char* name;
    u8  isSolid       : 1;
    u8  isTransparent : 1;
    ...
    u8  atlasSide;
    u8  atlasBottom;
    f32 hardness;        // врем€ добычи в секундах
    u32 toolFlags;       // какие инструменты эффективны
};

// –еестр блоков Ч заполн€етс€ на старте
class BlockRegistry {
public:
...
// √лобальные ID Ч согласованы с реестром
enum BlockId : u16 {
    AIR     = 0,
    STONE   = 1,
    DIRT    = 2,
