...
struct FeatureContext {
    ...
    /// Земля после рек, по колонкам чанка (индекс x * CHUNK_SIZE + z):
    /// первый не-твёрдый блок и признак «мокро» — вода, коса или
    /// береговая полоса у самой воды. Заполняет applyRivers; nullptr —
    ...
    /// что получит генерация соседнего чанка: дерево соседа не
    /// повиснет над руслом, вырезанным здесь.
    i32 groundAt(i32 lx, i32 lz, i32 wx, i32 wz, bool* wetOut = nullptr) const;
};
...
/// --- Реки и озёра из гидросети ---
///
/// Русла, долины, озёра, косы и дельты этого чанка — по сегментам,
/// которые гидрология заранее разложила по чанкам. Пишет в
