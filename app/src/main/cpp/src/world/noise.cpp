...
SimplexNoise::SimplexNoise(u64 seed) {
    ...
    // Смещение точки выборки. Без него решётка симплексов у всех
    // миров стоит на одном месте, и sample3D(0,0,0) равен нулю при
    // любой перестановке — см. подробности в noise.h.
    //
    // Диапазон небольшой: поля климата умножают координату на сотые
    // доли, и при смещении в тысячи шаг между соседними блоками
    // становится сравним с ulp. Сорока восьми хватает с запасом:
    // самое низкочастотное поле (континенты, 0.0004 на блок) уезжает
    // на сотню своих характерных размеров.
    offX_ = (f32)(splitmix64(state) % 100000u) * 0.00096f - 48.f;
    offY_ = (f32)(splitmix64(state) % 100000u) * 0.00096f - 48.f;
    offZ_ = (f32)(splitmix64(state) % 100000u) * 0.00096f - 48.f;
}
...
f32 SimplexNoise::sample3D(f32 xs, f32 ys, f32 zs) const {
    // 0. Смещение решётки. Своё у каждого зерна — иначе начало
    //    координат у всех миров приходится на узел решётки, где шум
    //    равен нулю по построению. См. noise.h.
    const f32 xin = xs + offX_;
    const f32 yin = ys + offY_;
    const f32 zin = zs + offZ_;
