...
/// Фартук вокруг ядра: 3072 блока. Свойства узла (сток, ширина,
/// фаза меандра) одинаковы в любой плитке, пока водосбор реки над ним
/// помещается в фартук. С рельефом по масштабам сток идёт по крупным
/// формам, водосборы вышли больше, и при 2048 блоках большие реки на
/// стыке плиток сдвигались на блок (rivercheck видел разрывы). Плитка
/// стоит дороже в полтора раза, но считается одна на 2048×2048 блоков.
constexpr i32 APRON_NODES = 96;
...
struct ColumnWater {
    ...
    bool frozen = false;
    /// 0..1: близость воды — река рядом (по ширине её долины и по
    /// размеру) или озеро. От неё зависят сырость земли, трава у
    /// берега и то, что там растёт. Считается и там, где сама колонка
    /// рекой не тронута (kind == None): долина шире русла.
    f32  near = 0.f;
};
