...
/// Превью мира: изометрический снимок области вокруг игрока,
/// снятый при выгрузке мира (выход в главное меню, выход из игры).
///
/// Файл назван по ЗЕРНУ, а не по слоту. Мир в игре — это зерно:
/// выход сохраняет его в слот автосохранения, а вручную его же
/// кладут и в другие слоты. Превью по слоту пришлось бы переносить
/// за каждым сохранением и чистить за каждым импортом, а превью по
/// зерну верно для любого слота, где лежит этот мир.
///
/// Рядом с сейвами: `<savesDir>/world_<зерно, 16 hex>.png`.
std::string worldPreviewPath(const std::string& savesDir, u64 seed);

/// Удалить превью миров, которых больше нет: ни в одном слоте из
/// `keepSeeds`. Возвращает, сколько файлов удалено.
///
/// Без этого удалённый мир оставлял бы свою картинку навсегда.
