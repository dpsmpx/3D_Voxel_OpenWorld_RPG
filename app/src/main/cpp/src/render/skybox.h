...
public:
    ...
    void destroy();
    /// set привязывается своим layout'ом. Когда небо рисовалось
    /// первым в кадре, оно не привязывало дескрипторы вовсе: шейдер
    /// читал матрицы из набора, оставшегося от интерфейса прошлого
    /// кадра, с несовместимым layout'ом.
