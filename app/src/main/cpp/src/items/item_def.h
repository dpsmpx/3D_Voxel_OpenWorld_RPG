...
struct ItemDef {
    ...
    /// Временная прибавка к атрибуту и на сколько секунд.
    ///
    /// effectDuration лежало здесь с самого начала и не читал его
    /// никто, а четыре эликсира не восстанавливали ни здоровья, ни
    /// маны, ни выносливости: applyConsumable возвращал «эффекта
    /// нет», предмет даже не тратился. Четыре предмета в игре не
    /// делали ровно ничего.
    ///
    ...
    /// Привязан к навыку: выбросить или продать нельзя, появляется
    /// и уходит вместе с навыком. Так в сумке лежат руны заклинаний
    /// Древа — держать заклинание в руке больше нечем.
    bool bound = false;

    /// Цвет сердцевины значка; 0 — цвет редкости. Иконки в игре —
    /// цветные квадраты, и две руны одной редкости иначе не
...
enum ItemId : u16 {
    ...
    // --- Руны заклинаний Древа ---
    ITEM_RUNE_ICE_NEEDLES = 160,
    ITEM_RUNE_FLAME       = 161,
