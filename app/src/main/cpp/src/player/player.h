...
/// ---- Цена смерти ----
///
/// Смерть стоит трёх вещей, и все три — разного рода:
///  * половина носимого золота остаётся на месте гибели и ждёт: её
///    можно вернуть, пройдя обратно через то, что убило;
///  * узелок, оставшийся с прошлой смерти и так и не поднятый,
///    пропадает — вторая смерть по дороге за ним стоит его целиком;
///  * пятая часть опыта, набранного на текущем уровне, теряется
///    насовсем. Уровень при этом не падает: отнимать выученное —
///    наказание уже не за смерть, а за игру.
///
/// Полосы у колодца полные: мёртвый и так заплатил, и заставлять его
/// ещё и стоять, пока отрастёт здоровье, — наказание ожиданием.
struct DeathCost {
    u64 gold = 0;   ///< остаётся на месте гибели
    u64 xp   = 0;   ///< теряется насовсем
};
...
public:
    ...
    /// Касание ячейки пояса прямо в игре (0..8).
    ///
    /// Ячейка становится выбранной: из неё ставятся блоки, её тратит
    /// кнопка USE. Оружие и руна надеваются сразу — в руке бывает
    /// только надетое, и выбирать его ради подсветки незачем: так пояс
    /// и становится сменой оружия на ходу. Касание уже выбранной
    /// ячейки — то же, что USE: выпить, съесть, бросить.
    /// @return что вышло; UseResult::Ok — ячейка просто выбрана
    items::UseResult tapHotbar(world::ChunkManager& world, u8 index);
    ...
    u64  lostGold      = 0;
    /// Сколько опыта унесла смерть. Тоже для слова на кадре смерти.
    u64  lostXp        = 0;
    /// Сколько золота пропало с прошлым, так и не поднятым узелком.
    u64  burnedGold    = 0;
...
private:
    ...
    /// Где игрок в последний раз стоял на твёрдом, не в жидкости.
    /// Туда ложится узелок, если смерть пришла в лаве или за краем
    /// мира: золото, утонувшее в лаве, вернуть нельзя, и «ждёт там,
    /// где вы пали» было бы враньём.
    glm::vec3 lastFooting_{0.f};
