...
// —чЄтчик дл€ отслеживани€ завершени€ группы задач.
// ћожно инкрементировать из любого потока, декрементировать по завершении.
class Counter {
public:
...
public:
    ...
    // ќтправить одиночную задачу. »з главного потока Ч в глобальную очередь.
    // »з воркера Ч в собственную (дл€ locality).
    void submit(JobFn fn, void* data = nullptr);

    // ќтправить с отслеживанием: counter->add() вызываетс€ автоматически.
    void submit(Counter* c, JobFn fn, void* data = nullptr);

    // ѕараллельный цикл [0, count), разбитый на куски не меньше minChunk.
    // fn(begin, end) вызываетс€ на воркерах. Ѕлокирует до завершени€.
    void parallelFor(u32 count, u32 minChunk,
                     const std::function<void(u32,u32)>& fn);

    u32 workerCount() const { return (u32)workers_.size(); }
    u32 currentWorkerId() const;   // UINT32_MAX дл€ внешних потоков
...
private:
    ...
    struct Job {
        JobFn fn;
        void* data;
        Counter* counter = nullptr;  // nullptr = без отслеживани€
    };
