...
void main() {
    ...
    //
    // Широкое сияние — шестая степень тремя умножениями, без
    // логарифма: считается на каждом пикселе неба, потому что заметно
    // далеко от солнца и обрезать его нечем.
    float d  = max(dot(dir, cam.sunDir.xyz), 0.0);
    float d2 = d * d;
    sky += sunTint * (d2 * d2 * d2) * 0.08 * above;

    // Диск и ореол — только вблизи солнца.
    //
    // Замер на устройстве: небо стоит 8.57 мс на полный экран, 3.44 нс
    // на пиксель — дороже фрагментной математики ландшафта. Пять
    // степеней на светила давали ноль почти везде и считались всюду.
    //
    // Порог выбран по величине отброшенного, а не на глаз, и считать
    // её надо по ХУДШЕМУ фону — чёрному небу, где прибавка x после
    // перевода в sRGB даёт 255*sqrt(x) уровней. Порог 0.80 этой
    // проверки не прошёл: ореол теряет там 6.7e-6, то есть 0.66
    // уровня, — край, а не запас. На 0.70 теряется 1.1e-8, это 0.03
    // уровня; диск — 1.2e-139.
    if (d > 0.70) {
        sky += sunTint * powSafe(d, 900.0) * 3.0 * above;    // диск
        sky += sunTint * powSafe(d, 48.0)  * 0.30 * above;   // ореол
    }

    // Луна — напротив солнца, видна ночью. Порог по тому же правилу:
    // на m = 0.90 ореол луны теряет 4.8e-9 — 0.02 уровня цвета,
    // диск 3.4e-110.
