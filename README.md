...
> покрыта 285 автоматическими проверками (`tools/hostcheck/run.sh`).
...
│   ├── preview/                    (рендер мира в картинку без устройства)
...
Четыре инструмента работают на обычном Linux-хосте, без Android-устройства
и без Vulkan. Они существуют потому, что цикл «собрать APK — поставить
на телефон — посмотреть — прислать журнал» занимает минуты, а ошибки,
которые он ловит, выглядят одинаково: «чёрный экран», «кривая
отрисовка», «тормозит».

```bash
./tools/hostcheck/run.sh          # компиляция, линковка, шейдеры, 285 проверок
./tools/preview/run.sh            # мир в PNG: геометрия, свет, LOD
...
**preview** — программный растеризатор, гоняющий настоящие генератор,
мешер и сборщик вершин. Нужен, чтобы увидеть геометрию и освещение, не
имея телефона: так нашлись дыры в дальнем рельефе от огрубления и
провал контраста в освещении. `--debug sky|ao|normal|material` показывает
...
по уровням детализации, сборку вершин, чтение вокселей, перерисовку
миникарты и поиск пути. Числа с хоста нельзя переносить на телефон один
в один, но соотношения переносятся — а нужны именно они, чтобы выбирать,
что чинить следующим. Именно этот инструмент показал, что путь в восемь
блоков состоит из четырёх тысяч точек, а перерисовка миникарты стоит
целого кадра.
